...
void	increment(t_ray *ray, t_c3d *c3d, double alpha) //# NOTA_2
{
	...
	if (strcmp(ray->cardinal_direction, "NE") == 0) //per ora ho toloto E per questinoi di testing
	{
		...
		while (1)
		{
			...
			{   
				printf("sez. x:\n");
				ray->end_point = calculation_of_end_point_to_check_along_SxTemp(c3d, ray, alpha);
				if (is_it_inside_map_perimeter(ray->end_point, c3d)) //#NOTA 1
				{
					if (is_it_passing_between_two_walls(c3d, ray, ray->end_point))
						break;
					if (is_it_a_wall_for_increment_x(c3d->map_fm_file.grid, ray->end_point, c3d, ray))
						break;
				}
				else
				{
					...
					{
						ray->end_point = calculation_of_end_point_to_check_along_SyTemp(c3d, ray, alpha); //ovvimente questa funzione aumenta sempre di un TILE_sIZE il dyTemporary
						if (is_it_a_wall_for_increment_y(c3d->map_fm_file.grid, ray->end_point, c3d, ray))
							break;
					}
			...
			else
			{
				...
				if (is_it_inside_map_perimeter(ray->end_point, c3d))
				{
					if (is_it_passing_between_two_walls(c3d, ray, ray->end_point))
						break;
					if (is_it_a_wall_for_increment_y(c3d->map_fm_file.grid, ray->end_point, c3d, ray))
				...
				else
				{
					...
					{
						ray->end_point = calculation_of_end_point_to_check_along_SxTemp(c3d, ray, alpha); //ovvimente questa funzione aumenta sempre di un TILE_sIZE il dyTemporary
						if (is_it_a_wall_for_increment_x(c3d->map_fm_file.grid, ray->end_point, c3d, ray))
							break;
					}
					break; //serve a far interrompere il while esterno
