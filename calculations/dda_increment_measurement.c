...
t_point calculation_of_end_point_to_check_along_SxTemp(t_c3d *c3d, t_ray *ray, double alpha)
{
    ...
    ray->DxTemp = fabs(ray->dx) + TILE_SIZE; //allora aumentiamo di TILE_SIZE il dx. Andiamo a salvare questo valroe in un DXtemporaneo
    printf("DxTemp = %f\n", ray->DxTemp);
...
t_point calculation_of_end_point_to_check_along_SyTemp(t_c3d *c3d, t_ray *ray, double alpha)
{
    ...
    point_to_return.x = 0;
    point_to_return.y = 0;
    ray->DyTemp = fabs(ray->dy) + TILE_SIZE; //aumento di TILE_SIZE
    printf("DyTemp = %f\n", ray->DyTemp);
