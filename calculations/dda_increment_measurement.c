...
void calculation_of_end_point_to_check_along_SxTemp(t_c3d *c3d, t_ray *ray, double alpha)
{
    ...
    ray->end_point_to_check.x = c3d->player.coordinates.x + fabs(ray->SxTemp) * cos(alpha); //grazie al calcolo del sx temporaneo ecco che posso calcoalrre il nuovo punto
    ray->end_point_to_check.y = c3d->player.coordinates.y + fabs(ray->SxTemp) * sin(alpha);
    printf("end_point_to_check: (%d, %d)\n", (int)ray->end_point_to_check.x, (int)ray->end_point_to_check.y);
