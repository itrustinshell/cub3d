...
void calculate_delta_x(t_c3d *c3d, t_ray *ray)
{
    if (strcmp(ray->cardinal_direction, "E") == 0)
        ray->delta_x = fabs(ray->first_impact_point.x - c3d->player.x); 
    else if (strcmp(ray->cardinal_direction, "NE") == 0 )
        ray->delta_x = fabs(ray->first_impact_point.x - c3d->player.x); 
    else if (strcmp(ray->cardinal_direction, "N") == 0 )
        ray->delta_x = fabs(ray->first_impact_point.x - c3d->player.x); //VERIFICA SE VA BENE dovrebbe essere indifferente se riferisce a lato delta_x o sx
    else if (strcmp(ray->cardinal_direction, "NW") == 0 )
        ray->delta_x = fabs(ray->first_impact_point.x - c3d->player.x - 1);
...
{
    if (strcmp(ray->cardinal_direction, "E") == 0)
        ray->delta_y = fabs(c3d->player.y - ray->first_impact_point.y + 1);
    if (strcmp(ray->cardinal_direction, "NE") == 0)
        ...
        ray->delta_y = fabs(c3d->player.y - ray->first_impact_point.y + 1); //dovrebbe essere indifferente se su o giu
    else if (strcmp(ray->cardinal_direction, "SW") == 0 )
        ray->delta_y = fabs(c3d->player.y - ray->first_impact_point.y); 
    else if (strcmp(ray->cardinal_direction, "S") == 0 )
        ray->delta_y = fabs(c3d->player.y - ray->first_impact_point.y);
    else if (strcmp(ray->cardinal_direction, "SE") == 0 )
        ray->delta_y = fabs(c3d->player.y - ray->first_impact_point.y); 
