...
void	color(int x, int y, t_c3d c3d, int color)
{
	int	i;
	int	bytes_per_pixel;

	bytes_per_pixel = c3d.win.bits_per_pixel / 8;
	//questo i va ai bits da colorare
	i = (y * c3d.win.size_line) + (x * bytes_per_pixel);
	c3d.win.data[i] = color & 0xFF;
	c3d.win.data[i + 1] = (color >> 8) & 0xFF;
	c3d.win.data[i + 2] = ((color >> 16) & 0xFF);
	c3d.win.data[i + 3] = ((color >> 24) & 0xFF);
}
