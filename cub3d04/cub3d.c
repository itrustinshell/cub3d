...
int main(int argc, char **argv)
{
	...
	c3d.win.img = mlx_new_image(c3d.win.mlx_connection, c3d.map.w * TILE_SIZE, c3d.map.h * TILE_SIZE);

	c3d.win.data = mlx_get_data_addr(c3d.win.img, &c3d.win.bits_per_pixel,
			&c3d.win.size_line, &c3d.win.endian);
	//design_map(c3d.win.mlx_connection, c3d.win.mlx_win, 0, 0, c3d.map.grid, c3d.map.w, c3d.map.h);

	int y = 0;
	int x = 0;
	while(y < c3d.map.h * TILE_SIZE)
	{
		x = 0;
		while (x < c3d.map.w * TILE_SIZE)
		{
			if (x < 10)
			{
				color(x, y , c3d, GREEN);
	...
	//print_fractal(argv, &c3d.win);
	mlx_put_image_to_window(c3d.win.mlx_connection, c3d.win.mlx_win, c3d.win.img, 0, 0);
